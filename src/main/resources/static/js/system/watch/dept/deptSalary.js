//搜索部门薪资
function searchDeptSalary(table) {
    if($('#login_name').val()===''&&$('#create_time').val()===''){
        table.reload('layui-table',{
            url:crx + '/salary/getPageDeptSalary?dept_id='+$.cache.get("user").user.dept_id,
            page: {
                curr: 1 //重新从第 1 页开始
            }
        });
        ...
        table.reload('layui-table',{
            url:crx + '/salary/searchDeptSalary?dept_id='+$.cache.get("user").user.dept_id,
            where: {
...
//导出部门薪资
function  exportDeptSalaryFile(excel){
    $.operate.get(crx+'/salary/getAllDeptSalary?dept_id='+$.cache.get("user").user.dept_id,exportExcel);
